...
class DetectionSimulator(Node):
    def __init__(self) -> None:
        ...
        num_targets = self.get_parameter("num_targets").get_parameter_value().integer_value
        spawn_radius = self.get_parameter("spawn_radius_m").get_parameter_value().double_value
        ...
        # Timer
        rate = self.get_parameter("detection_rate_hz").get_parameter_value().double_value
    ...
    def _create_detection(
        self, target_id: str, u: float, v: float, width: float, height: float, confidence: float
    ) -> Detection2D:
    ...
    def _on_timer(self) -> None:
        ...
        noise_px = self.get_parameter("detection_noise_pixels").get_parameter_value().double_value
        miss_prob = self.get_parameter("miss_probability").get_parameter_value().double_value
        target_size_m = self.get_parameter("target_size_m").get_parameter_value().double_value
        ...
        for target_id, tx, ty in self._targets:
            ...
            if margin <= u < img_w - margin and margin <= v < img_h - margin:
                ...
                # Publish detection
                det = self._create_detection(target_id, u, v, bbox_width, bbox_height, confidence)
                self._detection_pub.publish(det)

        # Occasional false positive
        fp_prob = self.get_parameter("false_positive_probability").get_parameter_value().double_value
        if random.random() < fp_prob:
            ...
            det = self._create_detection("noise", u, v, width, height, random.uniform(0.2, 0.5))
            self._detection_pub.publish(det)
