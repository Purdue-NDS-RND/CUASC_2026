...
class BackgroundDataLogger(Node):
    def __init__(self) -> None:
        ...
        self.session_dir = os.path.expanduser(f"~/Raycast_Session_{timestamp_str}")
        ...
        # Subscribers
        self.create_subscription(PoseStamped, "/mavros/local_position/pose", self._pose_callback, best_effort_qos)
        self.create_subscription(HomePosition, "/mavros/home_position/home", self._home_callback, transient_local_qos)
        self.create_subscription(CameraInfo, "/camera/camera_info", self._camera_info_callback, best_effort_qos)
        self.create_subscription(Image, "/camera/image_raw", self._image_callback, best_effort_qos)

        self.get_logger().info(f"📊 Background Logger Initialized.\nSaving session data to: {self.session_dir}")

    def _init_csv(self):
        headers = [
            "filename", "stamp_sec", "stamp_nanosec",
    ...
    def _camera_info_callback(self, msg: CameraInfo):
        ...
        # Save camera matrix parameter configurations directly to the session folder
        cam_data = {
        ...
        self.get_logger().info("📷 Camera calibration parameters saved to session directory.")

    def _image_callback(self, msg: Image):
        if self._current_pose is None:
            self.get_logger().warn("Skipping frame: Local pose telemetry stream not available yet.", throttle_duration_sec=3.0)
            return
        if self._home_position is None:
            self.get_logger().warn("Skipping frame: Home GPS anchor locked frame missing.", throttle_duration_sec=3.0)
            return
        ...
        try:
            ...
            row = [
                filename, msg.header.stamp.sec, msg.header.stamp.nanosec,
                pos.x, pos.y, pos.z,
                ori.x, ori.y, ori.z, ori.w,
                self._home_position.geo.latitude, self._home_position.geo.longitude
            ]
