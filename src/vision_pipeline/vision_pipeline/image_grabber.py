...
class ImageGrabber(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("wb_mode", 6)  # 1=Auto, 5=Daylight, 6=Cloudy
        self.declare_parameter(
            "image_publishing_rate", 1.0
        )  # Replaces your '-r' interval
        ...
        shutter = (
            self.get_parameter("shutter_speed").get_parameter_value().integer_value
        )
        wb = self.get_parameter("wb_mode").get_parameter_value().integer_value
        ...
        # Timer
        rate = (
            self.get_parameter("image_publishing_rate")
            .get_parameter_value()
            .double_value
        )
    ...
    def _get_pipeline(self, width, height, fps, shutter_speed_inv, wb_mode):
        ...
        if shutter_speed_inv > 0:
            ...
            self.get_logger().info(
                f"🔒 Locking Shutter to 1/{shutter_speed_inv}s ({exposure_ns} ns)"
            )
        else:
            exp_str = ""
            gain_str = ""
        ...
        return (
            f"nvarguscamerasrc sensor-id=0 {exp_str} {gain_str} {wb_str} ! "
