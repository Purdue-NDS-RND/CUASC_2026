...
class MissionLogger(Node):
    ...
    def _raycast_to_gps(self, u: float, v: float):
        ...
        # Extract RPY from the MAVROS quaternion.
        # MAVROS publishes ENU quaternion, so convert to NED Euler angles first.
        q = self._drone_pose.pose.orientation
        r_enu = R_scipy.from_quat([q.x, q.y, q.z, q.w])

        # ENU to NED body: swap and negate axes to match ArduPilot convention
        roll_enu, pitch_enu, yaw_enu = r_enu.as_euler("xyz", degrees=False)
        # ArduPilot NED: roll same, pitch negated, yaw = 90-yaw_enu
        roll = roll_enu
        pitch = -pitch_enu
